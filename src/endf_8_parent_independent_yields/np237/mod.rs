...
use crate::endf_8_parent_independent_yields::np237::fast_energy::np237_fast_fission_yield;
use crate::endf_8_parent_independent_yields::np237::high_energy::np237_high_fission_yield;
use crate::endf_8_parent_independent_yields::np237::thermal_energy::np237_thermal_fission_yield;


pub (crate) fn linear_linear_energy_interpolation_np237(
    ...
    if neutron_energy == thermal_neutron_energy {
        ...
        return np237_thermal_fission_yield(fission_product_nuclide);

    } else if neutron_energy == fast_neutron_energy {
        // fast energy case
        return np237_fast_fission_yield(fission_product_nuclide);

    } else if neutron_energy == high_neutron_energy {
        // high energy case
        return np237_high_fission_yield(fission_product_nuclide);
    ...
    let high_fission_yield = np237_high_fission_yield(
        fission_product_nuclide);
