...
use crate::endf_8_parent_independent_yields::pu240::fast_energy::pu240_fast_fission_yield;
use crate::endf_8_parent_independent_yields::pu240::high_energy::pu240_high_fission_yield;
use crate::endf_8_parent_independent_yields::pu240::thermal_energy::pu240_thermal_fission_yield;


/// note that pu240 does not have data at 14.0 MeV for ENDF VIII,
/// 
/// code will not work beyond 500 keV
pub (crate) fn linear_linear_energy_interpolation_pu240(
    ...
    if neutron_energy == thermal_neutron_energy {
        ...
        return pu240_thermal_fission_yield(fission_product_nuclide);

    } else if neutron_energy == fast_neutron_energy {
        // fast energy case
        return pu240_fast_fission_yield(fission_product_nuclide);
    ...
    let fast_fission_yield = pu240_fast_fission_yield(
        fission_product_nuclide);

    // this is for interpolating between thermal and fast range
    if neutron_energy < fast_neutron_energy {
        let thermal_fission_yield = pu240_thermal_fission_yield(
            fission_product_nuclide);
    ...
    let high_fission_yield = pu240_high_fission_yield(
        fission_product_nuclide);
