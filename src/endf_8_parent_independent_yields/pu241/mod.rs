...
use crate::endf_8_parent_independent_yields::pu241::high_energy::pu241_high_fission_yield;
...
pub (crate) fn linear_linear_energy_interpolation_pu241(
    ...
    if neutron_energy == thermal_neutron_energy {
        ...
        return pu241_thermal_fission_yield(fission_product_nuclide);

    } else if neutron_energy == fast_neutron_energy {
        // fast energy case
        return pu241_fast_fission_yield(fission_product_nuclide);
    ...
    let high_fission_yield = pu241_high_fission_yield(
        fission_product_nuclide);
